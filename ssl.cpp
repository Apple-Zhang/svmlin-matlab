	 ...
	 struct vector_double *Outputs)
{
  ...
  int *J = Subset->vec;
  double *val = Data->val;
  unsigned long long *row = Data->rowptr;
  unsigned long long *col = Data->colind;
  double *Y = Data->Y;
  double *C = Data->C;
  int n  = Data->n;
...
void GetLabeledData(struct data *D, const struct data *Data)
{
  ...
  D->val    = new double[nz];
  D->colind = new unsigned long long[nz]; 
  D->rowptr = new unsigned long long[rowptrs_+1];
  nz=0;
...
void SetData(struct data *a, int m,int n, int l,int u, int nz, double *VAL, unsigned long long *R, unsigned long long *C, double *Y, double *COSTS)
{
  a->m=m;
  a->u=u;
  a->l=m-u;
